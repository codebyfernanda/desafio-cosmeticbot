...
## ✸ Arquitetura do Sistema & Modelos Utilizados
...
## ✸ Estratégia de *Quality Assurance* (QA) & Métricas
...
1.  **1ª Métrica - *Answer Relevancy* (≥ 0.7):** Mensura a capacidade do assistente de compreender e responder diretamente à dor do usuário sem desvios de escopo.
2.  **2ª Métrica - *Faithfulness* (≥ 0.8):** Mede a fidelidade ao contexto. Penaliza alucinações de formulações, preços ou ingredientes não previstos no catálogo oficial.
3.  **3ª Métrica - *G-Eval* de Conformidade de *Claims* (≥ 0.8):** Audita *guardrails* clínicos. O modelo deve evitar promessas de cura, recusar tratamentos para patologias e direcionar o usuário a um dermatologista quando necessário.
...
## ✸ Sessão Exploratória & Diagnóstico da *Baseline*
A fase inicial consistiu em uma sessão de testes exploratórios de 1 hora de duração após as configurações iniciais do ambiente, com o objetivo de "estressar" o *prompt* original do *chatbot* fornecido - que atuava sem nenhum tipo de *guardrails* - para mapear vulnerabilidades do *LLM*. O resultado disso foi a execução de 15 casos de teste (com taxas altíssimas de falhas), evidenciando riscos consideráveis à segurança do usuário e, mais especificamente, à percepção da marca de cosméticos. Dito isso, é importante ressaltar que o *bot* apresentou alucinação de dados e quebra de identidade da marca.
...
## ✸ Avaliação & Refatoração
A *baseline* do projeto demonstrou 100% de falha nos testes iniciais (logo após a configuração do ambiente), evidenciando riscos à percepção da marca e à saúde do usuário. A reestruturação focou na aplicação direta de princípios no *prompt* do sistema: delimitação clara de persona, instruções literais em caso de doenças ("Consulte um médico dermatologista") e bloqueio de conhecimentos externos.

## ✸ Estrutura do *Golden Dataset*
Depois disso, era a hora de estruturar um conjunto de dados a partir de uma sessão exploratória inicial que identificou falhas críticas (alucinações automotivas, culinárias e diagnósticos médicos) no *prompt* base. Os casos foram categorizados em:
...
### ✸ Impacto da Temperatura & Fadiga Cognitiva
